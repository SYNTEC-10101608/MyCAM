...
namespace MyCAM.Editor
{
	internal class ToolVectorAction : IndexSelectAction
	{
			...
			: base( dataManager, viewer, treeView, viewManager, pathID )
		{
			...
			m_ContourCache = ( ( m_DataManager.ObjectMap[ m_PathID ] as ContourPathObject ).ContourCache ) as ContourCache;
		...
		protected override void ViewerMouseClick( MouseEventArgs e )
		{
			...
			// modify tool vector
			bool isModified = m_ContourCache.GetToolVecModify( nIndex, out double angleA_deg, out double angleB_deg );
		...
		ContourCache m_ContourCache = null;
