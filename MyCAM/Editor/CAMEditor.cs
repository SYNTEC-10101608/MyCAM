		...
		void ShowLeadLine_New()
		{
			...
			foreach( CAMData camData in m_DataManager.GetCAMDataList() ) {
				...
				if( camData.LeadLineParam.LeadIn.Type == LeadLineType.Line ) {
					Geom_Curve lineCurve = LeadHelper.BuildStraightLeadLine_New( camData.BreakedCAMSegmentList[ 0].StartPoint.CADPoint, camData.BreakedCAMSegmentList[ 0 ].StartPoint.ToolVec, true, camData.LeadLineParam.LeadIn.Length, camData.LeadLineParam.LeadIn.Angle, camData.LeadLineParam.IsChangeLeadDirection, camData.IsReverse, out gp_Pnt leadLineEndPoint, out gp_Dir leadDir );
					if( lineCurve == null ) {
						break;
					}
				...
				if( camData.LeadLineParam.LeadIn.Type == LeadLineType.Arc ) {
					Geom_Curve arcCurve = LeadHelper.BuildArcLead_New( camData.BreakedCAMSegmentList[ 0 ].StartPoint.CADPoint, camData.BreakedCAMSegmentList[ 0].StartPoint.ToolVec, true, camData.LeadLineParam.LeadIn.Length, camData.LeadLineParam.LeadIn.Angle, camData.LeadLineParam.IsChangeLeadDirection, camData.IsReverse, out gp_Pnt leadLineEndPoint, out _, out gp_Dir leadDir );
					if( arcCurve == null ) {
						break;
					}
				...
				if( camData.LeadLineParam.LeadOut.Type == LeadLineType.Line ) {
					Geom_Curve lineCurve = LeadHelper.BuildStraightLeadLine_New( camData.BreakedCAMSegmentList[ 0].StartPoint.CADPoint, camData.BreakedCAMSegmentList[0 ].StartPoint.ToolVec, false, camData.LeadLineParam.LeadOut.Length, camData.LeadLineParam.LeadOut.Angle, camData.LeadLineParam.IsChangeLeadDirection, camData.IsReverse, out gp_Pnt leadLineEndPoint, out gp_Dir leadDir );
					if( lineCurve == null ) {
						break;
					}
					AIS_Shape lineShape = CurveToAIS( lineCurve, Quantity_NameOfColor.Quantity_NOC_GREENYELLOW );
				...
				if( camData.LeadLineParam.LeadOut.Type == LeadLineType.Arc ) {
					Geom_Curve arcCurve = LeadHelper.BuildArcLead_New( camData.BreakedCAMSegmentList[ 0].StartPoint.CADPoint, camData.BreakedCAMSegmentList[0 ].StartPoint.ToolVec, false, camData.LeadLineParam.LeadOut.Length, camData.LeadLineParam.LeadOut.Angle, camData.LeadLineParam.IsChangeLeadDirection, camData.IsReverse, out gp_Pnt leadLineEndPoint, out _, out gp_Dir leadDir );
