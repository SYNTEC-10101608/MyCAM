...
namespace MyCAM.Helper
{
    ...
    {
        public static List<CADPoint> ApplyContourEdit(
            List<CADPoint> cadPointList,
            IReadOnlyDictionary<int, ContourEditData> contourEditMap,
            bool isClosed )
        {
            ...
            if( contourEditMap == null || contourEditMap.Count == 0 ) {
                return result;
            }

            // build a working copy of the map so we can add boundary control points
            Dictionary<int, ContourEditData> workMap =
                contourEditMap.ToDictionary( kvp => kvp.Key, kvp => kvp.Value.Clone() );
        ...
        static void AddBoundaryControlPoints(
            ref Dictionary<int, ContourEditData> workMap,
            ...
            List<int> sortedKeys )
        {
            ...
            ContourEditData dataFirst = workMap[ firstCtrl ];
            ContourEditData dataLast = workMap[ lastCtrl ];
        ...
        static void ApplyDisplacementInterpolation(
            List<CADPoint> pointList,
            IReadOnlyDictionary<int, ContourEditData> workMap )
        {
            List<int> sortedKeys = workMap.Keys.ToList();
            sortedKeys.Sort();
            ...
            foreach( KeyValuePair<int, ContourEditData> entry in workMap ) {
            ...
            int endIdx )
        {
            ...
            ContourEditData startData = workMap[ startIdx ];
            ContourEditData endData = workMap[ endIdx ];
        ...
        static void ApplyDisplacement( CADPoint cadPoint, ContourEditData disp )
        {
        ...
        // Linear interpolation between two ContourEditData at parameter t.
        static ContourEditData Lerp( ContourEditData a, ContourEditData b, double t )
        {
            return new ContourEditData(
                a.DX + ( b.DX - a.DX ) * t,
                a.DY + ( b.DY - a.DY ) * t,
                a.DZ + ( b.DZ - a.DZ ) * t );
        }
